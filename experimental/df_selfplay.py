...
def main():
    ...
    def game_end(batch):
        ...
        # print("In game end")
        wr = batch.GC.getGameStats().getWinRateStats()
        win_rate = 100.0 * wr.black_wins / wr.total_games if wr.total_games > 0 else 0.0
        print("%s B/W: %d/%d. Black winrate: %.2f (%d)" %
                (str(datetime.now()), wr.black_wins, wr.white_wins, win_rate, wr.total_games))
        if args.suicide_after_n_games > 0 and wr.total_games >= args.suicide_after_n_games:
            print("#suicide_after_n_games: %d, total_games: %d" % (args.suicide_after_n_games, wr.total_games))
            loop_end = True
    ...
    if args.eval_model_pair:
        ...
        GC.GC.setRequest(int(black), int(white), env['game'].options.resign_thres, 1)
