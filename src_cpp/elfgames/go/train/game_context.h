...
class GameContext {
 public:
    ...
    if (performTraining) {
      ...
      for (int i = 0; i < numGames; ++i) {
        games_.emplace_back(new GoGameSelfPlay(
            ...
            contextOptions,
            options,
            evalCtrl_->getDispatcher(),
            evalCtrl_->getNotifier()));
      }
    }

    ThreadedDispatcher* dispatcher =
        evalCtrl_ != nullptr ? evalCtrl_->getDispatcher() : nullptr;
  ...
  void setRequest(
    ...
    evalCtrl_->getDispatcher()->sendToThread(request);
  }
 ...
 private:
  ...
  void loadOfflineSelfplayData(const GameOptions& options) {
    ...
    auto thread_main = [&options, this, &count](size_t idx) {
